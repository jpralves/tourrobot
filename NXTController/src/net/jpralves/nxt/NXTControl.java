...
public class NXTControl {
	...
	private static final String TAG = NXTControl.class.getSimpleName();

	private int speed = 100;

	private NXTProcessBTInput bti;
	private NXTProcessBTOutput bto;
	...
	private NXTConnector nxtConn = null;
	 ...
	 */
	public NXTControl(String nxtName, int motorLPos, int motorRPos, int motorFlags, int initSpeed,
			int minDistance, int touchPos, int colorPos, int sonicleftPos, int sonicrightPos,
			int sonicfrontPos, NXTControllerApp app) {
	...
	public boolean connectToNXT() {
		setupNXJCache();
		nxtConn = connect(/* CONN_TYPE.LEJOS_PACKET */);
		if (nxtConn != null) {
			...
			bti = new NXTProcessBTInput(app, dis);
			bto = new NXTProcessBTOutput(app, dos);
